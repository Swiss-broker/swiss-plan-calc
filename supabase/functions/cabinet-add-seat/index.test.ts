...
const profiles: Record<string, { plan: string; email: string }> = {
  "director-A": { plan: "cabinet", email: "alice@cabinet-a.ch" },
  "director-internal": { plan: "internal", email: "founder@swissbrokerpro.ch" },
  "director-noemail": { plan: "cabinet", email: "fallback@cabinet-a.ch" },
};
...
function mockFetch(input: string | URL | Request, init?: RequestInit): Promise<Response> {
  ...
  if (url.includes("/rest/v1/profiles?id=eq.") && url.includes("select=plan,email")) {

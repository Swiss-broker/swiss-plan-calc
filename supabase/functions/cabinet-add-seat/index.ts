...
export async function handleCabinetAddSeatRequest(req: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    // récupère aussi l'email de secours si le JWT n'en porte pas, ainsi
    // que le rôle cabinet de l'appelant pour le contrôle d'autorité
    // ci-dessous.
    const inviterProfileRes = await fetch(
      `${supabaseUrl}/rest/v1/profiles?id=eq.${inviterId}&select=plan,email,cabinet_role,cabinet_root_id`,
      { headers: { apikey: supabaseKey, Authorization: `Bearer ${supabaseKey}` } },
    );
    const inviterProfiles = await inviterProfileRes.json();
    const inviterProfile = inviterProfiles[0];
    const isInternal = inviterProfile?.plan === "internal";
    const inviterEmail: string | undefined = caller.email ?? inviterProfile?.email;
    ...
    // quel compte authentifié pourrait injecter une invitation dans le
    // cabinet de quelqu'un d'autre — et, pire, si l'email cible correspond
    // à un compte existant, le re-rattacher de force à ce cabinet (voir
    // plus bas : mise à jour immédiate du profil).
    const inviterHasAuthority =
      isInternal ||
      (inviterProfile?.cabinet_role === "root_director" && cabinetRootId === inviterId) ||
      (inviterProfile?.cabinet_role === "director" && cabinetRootId === inviterProfile?.cabinet_root_id);
